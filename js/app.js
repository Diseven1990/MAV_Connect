...
function render() {
  ...
  const normais = noticias.filter((noticia) => !noticia.destaque);

  els.featured.innerHTML = destaques.length
    ? destaques.map((noticia) => cardTemplate(noticia, true)).join('')
    : '<p class="muted-note">Ainda não existem destaques publicados.</p>';
