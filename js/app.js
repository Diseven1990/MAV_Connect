...
function bindEvents() {
  if (els.search) { els.search.addEventListener('input', (event) => { state.pesquisa = event.target.value.trim().toLowerCase(); render();}); }
  ...
  document.querySelectorAll('.nav-item[data-view]').forEach((item) => {
    item.addEventListener('click', () => {
      ...
      if (view === 'featured') document.getElementById('featuredSection').scrollIntoView({ behavior: 'smooth', block: 'start' });
      if (view === 'search') { const termo = prompt('Pesquisar notícias:'); if(termo!==null){ state.pesquisa=termo.toLowerCase(); render();}}
    });
  });
  ...
  window.addEventListener('keydown', (event) => {
    if (event.key === 'Escape' && !els.modal.classList.contains('hidden')) fecharModal();
  });
}
...
async function carregarNoticias() {
  try {
    ...
    console.error(error);
    els.resultCount.textContent = 'Erro ao carregar';
...
function render() {
  ...
  const destaques = noticias.filter((n)=>{ if(!n.destaque) return false; const d=new Date(n.data_publicacao||n.created_at); return ((Date.now()-d.getTime())/86400000)<=5;});
  const normais = noticias.filter((n)=> !destaques.includes(n));

document.getElementById('featuredSection').style.display=destaques.length?'block':'none'; els.featured.innerHTML=destaques.map((n)=>cardTemplate(n,true)).join('');

  els.feed.innerHTML = normais.length
    ? normais.map((noticia) => cardTemplate(noticia, false)).join('')
    : '';
